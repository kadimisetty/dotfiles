...
function! ReplaceCargoCommand(command)
    " Runs validated cargo command. If a window alreadyb exists from a
    " previous cargo command, it is repalced i.e. it's buffer is deleted and a
    " fresh cargo command via `rust-lang/rust.vim` is run on a new term.
    ...
    if index(valid_cargo_commands, a:command) < 0
        ...
        let windows_in_current_tabpage = tabpagebuflist(tabpagenr())
        for i in windows_in_current_tabpage
            if bufname(i) =~# ('^!cargo ' . a:command)
                execute ('bdelete ' . i)
            endif
        endfor
